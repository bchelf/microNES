...
| `episode` | `dict` | `VecMonitor` | Contains `r`, `l`, `t` at episode end only | Only present in the final step info of an episode |
...
`NewMaxXWrapper.max_x_seen` initializes to `0.0` at construction time. Since training uses `SubprocVecEnv`, each of the 8 workers has its own independent `max_x_seen` that starts at `0.0`. There is no cross-worker sharing of the frontier — env 0's value is used as the representative global frontier in diagnostics.
...
| `wrappers.py` | `NewMaxXWrapper`, `SurvivalBonusWrapper`, `DeathPenaltyWrapper`. Full BUGFIX audit at top of file. |
| `eval_callback.py` | `DiagnosticsCallback`, `EntropySchedulerCallback`, `CurriculumStageCallback`, `EvalVideoCallback`. |
| `train_smb_rl.py` | Training entry point. PPO config, wrapper assembly, callback assembly. |
