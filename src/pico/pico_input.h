...
//   NES pin 1 (VCC)   → 3.3 V
//   NES pin 2 (CLOCK) → GP26 (output from Pico)
//   NES pin 3 (LATCH) → GP27 (output from Pico)
//   NES pin 4 (DATA)  → GP28 (input to Pico; pull-up enabled in firmware)
//   NES pin 7 (GND)   → GND
// The controller now lives on one contiguous 3-pin block clear of both TFT
// display mappings and the MAX98357 audio backend.
#define PICO_NES_CTL_CLOCK  26u
#define PICO_NES_CTL_LATCH  27u
#define PICO_NES_CTL_DATA   28u
