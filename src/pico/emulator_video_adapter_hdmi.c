/*
 * emulator_video_adapter_hdmi.c — emulator-to-HDMI adapter.
 *
 * Mirrors the shape of emulator_video_adapter_tft.c.  The HSTX driver
 * exposes a present_frame() that is non-blocking (it copies the indexed
 * buffer into an internal "present" slot and the DMA-driven line ISR
 * picks it up), so the adapter just calls through.
 */
...
#error "The HDMI Pico target requires MICRONES_ENABLE_FRAMEBUFFER=1"
#endif

static void emulator_video_adapter_set_error(PicoEmulatorVideoAdapter *adapter,
                                             const char *message) {
...
bool emulator_video_adapter_step_frame(PicoEmulatorVideoAdapter *adapter) {
    ...
    if (!adapter->initialized) {
        emulator_video_adapter_set_error(adapter,
            "emulator video adapter is not initialized");
        return false;
    }
