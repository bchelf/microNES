// Standalone RP2350 HSTX/DVI bring-up target for the microNES v0.1 board.
//
...
static uint8_t __attribute__((aligned(4)))
    framebuf[MODE_H_ACTIVE_PIXELS * MODE_V_ACTIVE_LINES];
...
static void fill_test_pattern(void) {
    for (uint y = 0; y < MODE_V_ACTIVE_LINES; ++y) {
        for (uint x = 0; x < MODE_H_ACTIVE_PIXELS; ++x) {
            uint8_t r;
            uint8_t g;
            uint8_t b;
            ...
            framebuf[y * MODE_H_ACTIVE_PIXELS + x] = colour_rgb332(r, g, b);
        }
    }
}
