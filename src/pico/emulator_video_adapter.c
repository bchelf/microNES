...
// NES palette indices mapped to 8 luma levels for composite NTSC output.
// Each brightness row uses a consistent luma for chromatic entries ($x1-$xC),
// matching the real NES PPU voltage levels:
//   Row 0 ($0x): dark     — luma ≈ 0.42V → DARK(1)
//   Row 1 ($1x): medium   — luma ≈ 0.58V → MID(3)
//   Row 2 ($2x): bright   — luma ≈ 0.83V → BRIGHT(5)
//   Row 3 ($3x): pastel   — luma ≈ 0.99V → VERY_BRIGHT(6)
// Gray column ($x0) uses the row's "high" voltage; $xD/$xE/$xF are black.
const uint8_t k_emulator_video_palette_to_gray[64] = {
    ...
    /* Row 3: $30-$3F — pastel (very bright, low saturation) */
