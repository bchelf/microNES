...
#define HDMI_AUDIO_QUEUE_TARGET      192u
#define HDMI_AUDIO_SERVICE_BUDGET    8u
...
static uint32_t __attribute__((aligned(4)))
    s_hdmi_silence_island[HDMI_ONE_PACKET_ISLAND_WORDS];
...
static int16_t s_hdmi_audio_last_l;
static int16_t s_hdmi_audio_last_r;
...
static void hdmi_pull_stereo_samples(int16_t out[8], uint32_t want_frames,
    ...
    if (take > 0u) {
        s_hdmi_audio_last_l = out[(take - 1u) * 2u + 0u];
        s_hdmi_audio_last_r = out[(take - 1u) * 2u + 1u];
    }
    /* If under-running, pad with the last sample (or zero if buffer empty). */
    if (take < want_frames) {
        int16_t pad_l = take ? out[(take - 1u) * 2u + 0u] : s_hdmi_audio_last_l;
        int16_t pad_r = take ? out[(take - 1u) * 2u + 1u] : s_hdmi_audio_last_r;
...
static void hdmi_build_silence_island(void) {
    HdmiPacket packet;
    int16_t samples[8] = {0};
    uint32_t saved_frame_no = s_hdmi_audio_frame_no;

    hdmi_pkt_make_audio_sample(&packet, samples, 4u, &saved_frame_no);
    hdmi_di_emit_island(&packet, 1u, 1u, 0u, s_hdmi_silence_island);
}
...
bool video_hstx_init(void) {
    ...
    s_hdmi_audio_last_l = 0;
    s_hdmi_audio_last_r = 0;
    s_hdmi_pcm_head = 0u;
    s_hdmi_pcm_tail = 0u;
...
void video_hstx_print_diag(void) {
    printf("[hstx] sys=%lu hstx=%lu pixel=%lu CTS=%u N=%u tone=%u pcm_level=%lu q=%lu underruns=%lu overruns=%lu refills=%lu missed=%lu pkts=%lu\n",
