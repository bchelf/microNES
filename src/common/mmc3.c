...
static void mmc3_update_prg_banks(NesCartridge *cart) {
    ...
    uint8_t last = (uint8_t)(bank_count_8k - 1u);
    uint8_t second_last = (uint8_t)(bank_count_8k - 2u);
...
void mmc3_cpu_write(NesCartridge *cart, uint16_t addr, uint8_t value) {
    ...
    case 0xa000u:
        ...
        break;
    case 0xa001u:
        /* Experimental: ignore WRAM protect/enable for now. */
        break;
    case 0xc000u:
...
void mmc3_ppu_a12_update(NesCartridge *cart, bool high) {
    if (high && !cart->mmc3_a12_level) {
        mmc3_scanline_tick(cart);
    }
    cart->mmc3_a12_level = high;
}
